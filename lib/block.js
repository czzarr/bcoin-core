...
function Block(network, data, subtype) {
    ...
    this.parser = bcoin.protocol.parser(network)

    this.type = 'block';
    this.subtype = subtype;
    this.version = data.version;
    this.prevBlock = utils.toHex(utils.toArray(data.prevBlock).reverse());
...
Block.prototype.hashLE = function hash(enc) {
    // Hash it
...
Block.prototype.hash = function hash(enc) {
    // Hash it
    if (!this._hash) {
      this.__hash = utils.dsha256(this.abbr()).reverse();
      this._hash = utils.toHex(this.__hash)
    }
    return enc === 'hex' ? this._hash : this.__hash;
};
...
Block.fromJSON = function fromJSON(network, json) {
    ...
    var parser = new bcoin.protocol.parser(network);
