...
function Framer(options) {
    ...
    this.agent = utils.toArray(options.agent || '/bcoinx:' + version + '/');
    this.agent = this.agent.slice(0, 0xfc);
    this.constants = bcoin.constants(this.network)
}
...
Framer.prototype.header = function header(cmd, payload) {
    ...
    // Magic value
    writeU32(h, this.constants.magic, 0);
...
Framer.prototype.version = function version(packet) {
    ...
    // Version
    off += writeU32(p, this.constants.version, off);

    // Services
    off += writeU32(p, this.constants.services.network, off);
...
Framer.prototype._inv = function _inv(command, items) {
    ...
    for (var i = 0; i < items.length; i++) {
        // Type
        off += writeU32(res, this.constants.inv[items[i].type], off);

        // Hash
        var hash = items[i].hash;
        if (typeof hash === 'string') hash = utils.toArray(hash, 'hex');
...
Framer.prototype.filterLoad = function filterLoad(bloom, update) {
    ...
    // nFlags
    after[8] = this.constants.filterFlags[update];
...
Framer.prototype.getBlocks = function getBlocks(hashes, stop) {
    var p = [];
    writeU32(p, this.constants.version, 0);
