...
function Parser(network) {
    ...
    this.waiting = 24;
    this.packet = null;
    this.constants = bcoin.constants(network)
}
...
Parser.prototype.feed = function feed(data) {
    ...
    while (this.pendingTotal >= this.waiting) {
        ...
        for (var i = 0, off = 0, len = 0; off < chunk.length; i++) {
            ...
            else if (!this.pending[0].slice && this.pending[0] instanceof Uint8Array) this.pending[0] = new Uint8Array(this.pending[0].buffer.slice(len));
...
Parser.prototype.parse = function parse(chunk) {
    if (this.packet === null) {
        ...
        if (readU32(utils.checksum(this.packet.payload)) !== this.packet.checksum) return this._error('Invalid checksum');
        this.packet.payload = this.parsePayload(this.packet.cmd,
        this.packet.payload);
        if (this.packet.payload) this.emit('packet', this.packet);
...
Parser.prototype.parseHeader = function parseHeader(h) {
    ...
    if (i === 12) return this._error('Not NULL-terminated cmd');
...
Parser.prototype.parsePayload = function parsePayload(cmd, p) {
    if (cmd === 'version') return this.parseVersion(p);
    else if (cmd === 'getdata' || cmd === 'inv' || cmd === 'notfound') return this.parseInvList(p);
    else if (cmd === 'merkleblock') return this.parseMerkleBlock(p);
    else if (cmd === 'block') return this.parseBlock(p);
    else if (cmd === 'tx') return this.parseTX(p);
    else if (cmd === 'reject') return this.parseReject(p);
    else if (cmd === 'addr') return this.parseAddr(p);
    else return p;
};

Parser.prototype.parseVersion = function parseVersion(p) {
    if (p.length < 85) return this._error('version packet is too small');
...
Parser.prototype.parseInvList = function parseInvList(p) {
    ...
    p = p.slice(count.off);
    count = count.r;
    if (p.length < count * 36) return this._error('Invalid getdata size');

    var items = [];
    for (var i = 0, off = 0; i < count; i++, off += 36) {
        items.push({
            type: this.constants.invByVal[readU32(p, off)],
...
Parser.prototype.parseMerkleBlock = function parseMerkleBlock(p) {
    if (p.length < 86) return this._error('Invalid merkleblock size');

    var hashCount = readIntv(p, 84);
    var off = hashCount.off;
    hashCount = hashCount.r;
    if (off + 32 * hashCount + 1 > p.length) return this._error('Invalid hash count');
    ...
    if (off + flagCount > p.length) return this._error('Invalid flag count');
...
Parser.prototype.parseBlock = function parseBlock(p) {
    if (p.length < 81) return this._error('Invalid block size');
...
Parser.prototype.parseTXIn = function parseTXIn(p) {
    if (p.length < 41) return this._error('Invalid tx_in size');

    var scriptLen = readIntv(p, 36);
    var off = scriptLen.off;
    scriptLen = scriptLen.r;
    if (off + scriptLen + 4 > p.length) return this._error('Invalid tx_in script length');
...
Parser.prototype.parseTXOut = function parseTXOut(p) {
    if (p.length < 9) return this._error('Invalid tx_out size');

    var scriptLen = readIntv(p, 8);
    var off = scriptLen.off;
    scriptLen = scriptLen.r;
    if (off + scriptLen > p.length) return this._error('Invalid tx_out script length');
...
Parser.prototype.parseTX = function parseTX(p) {
    ...
    var off = inCount.off;
    inCount = inCount.r;
    if (inCount < 0) return this._error('Invalid tx_in count (negative)');
    if (off + 41 * inCount + 5 > p.length) return this._error('Invalid tx_in count (too big)');
    ...
    for (var i = 0; i < inCount; i++) {
        ...
        if (off + 5 > p.length) return this._error('Invalid tx_in offset');
    }

    var outCount = readIntv(p, off);
    var off = outCount.off;
    outCount = outCount.r;
    if (outCount < 0) return this._error('Invalid tx_out count (negative)');
    if (off + 9 * outCount + 4 > p.length) return this._error('Invalid tx_out count (too big)');
    ...
    for (var i = 0; i < outCount; i++) {
        ...
        if (off + 4 > p.length) return this._error('Invalid tx_out offset');
    }
...
Parser.prototype.parseReject = function parseReject(p) {
    if (p.length < 3) return this._error('Invalid reject size');

    var messageLen = readIntv(p, 0);
    var off = messageLen.off;
    messageLen = messageLen.r;
    if (off + messageLen + 2 > p.length) return this._error('Invalid reject message');
    ...
    if (off + reasonLen > p.length) return this._error('Invalid reject reason');
...
Parser.prototype.parseAddr = function parseAddr(p) {
    if (p.length < 31) return this._error('Invalid addr size');
    ...
    for (var i = 0; i < count; i++) {
        ...
        ipv4 = ((ipv4 >> 24) & 0xff) + '.' + ((ipv4 >> 16) & 0xff) + '.' + ((ipv4 >> 8) & 0xff) + '.' + ((ipv4 >> 0) & 0xff);
