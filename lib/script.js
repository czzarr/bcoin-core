...
script.getOutputType = function getOutputType (s) {
  s = script.decode(s)
  if (Array.isArray(script.isPubkeyhash(s))) {
    return 'pubkeyhash'
  } else if (Array.isArray(script.isSimplePubkeyhash(s))) {
    return 'pubkey'
  } else if (script.isScripthash(s)) {
    return 'scripthash'
  } else if (parsed = script.isMultisig(s)) {
    return 'multisig' + parsed.m.toString() + 'of' + parsed.n.toString()
...
// s should be a decoded script
// opts should contain network, type and is_output parameters
script.getAddressesByType = function getAddressesByType (s, opts) {
  switch (true) {
    case /^pubkey$/.test(opts.type):
      // since the public key is only accessible in the output script
      // we never extract it from an input script
      return [utils.hash2addr(utils.ripesha(s[0]), opts.network)]
      break
    case /^pubkeyhash$/.test(opts.type):
      if (opts.is_output) {
        return [utils.hash2addr(s[2], opts.network)]
      } else {
        return [utils.hash2addr(utils.ripesha(s[1]), opts.network)]
      }
      break
    case /^scripthash$/.test(opts.type):
      if (opts.is_output) {
        return [utils.hash2scriptaddr(s[1], opts.network)]
      } else {
        // we hash the redeem script to find the address
        return [utils.hash2scriptaddr(utils.ripesha(s[s.length - 1]), opts.network)]
      }
      break
    case /^multisig/.test(opts.type):
      // since the public keys are only accessible in the output script
      // we never extract them from an input script
      var n = s[s.length - 2];
      var keys = s.slice(1, 1 + n);
      return keys.map(function (k) {
        return utils.hash2addr(utils.ripesha(k), opts.network)
      })
      break
  }
}

script.getReadableScript = function getReadableScript (s, opts) {
  s = s.slice()
  switch (true) {
    case /^pubkey$/.test(opts.type):
      if (opts.is_output) return [utils.toHex(s[0]), s[1]].join(' ')
      else return utils.toHex(s[0])
      break
    case /^pubkeyhash$/.test(opts.type):
      if (opts.is_output) {
        s[2] = utils.toHex(s[2])
        ...
        return s.map(utils.toHex.bind(s)).join(' ')
      }
      break
    case /^scripthash$/.test(opts.type):
      if (opts.is_output) {
        ...
        s = s.map(function (e) {
          if (Array.isArray(e)) return utils.toHex(e)
          else return e
        })
        if (s[0] === '') s[0] = '0' // OP_0 = [] and 0
        return s.join(' ')
      }
      break
    case /^multisig/.test(opts.type):
      if (opts.is_output) {
        var m = s[0]
        var n = s[s.length - 2]
        var keys = s.slice(1, s.length - 2).map(utils.toHex.bind(s)).join(' ')
        return [m, keys, n, s[s.length-1]].join(' ')
      } else {
        s = s.map(utils.toHex.bind(s))
        s[0] = '0'
        return s.join(' ')
      }
      break
    case /^nulldata/.test(opts.type):
      return [s[0], utils.toHex(s[1])].join(' ')
      break
    default:
      s = s.map(function (chunk) {
        if (typeof chunk === 'string') return chunk
        if (Array.isArray(chunk)) return utils.toHex(chunk)
