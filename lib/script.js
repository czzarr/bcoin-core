...
script.getOutputType = function (s) {
...
script.getAddress (s) {
  s = bcoin.script.decode(s)
  if (Array.isArray(bcoin.script.isPubkeyhash(s))) {
    return utils.hash2addr(s[2], config.network)
  }
  else if (Array.isArray(bcoin.script.isSimplePubkeyhash(s))) {
    return utils.hash2addr(utils.ripesha(s[0]), config.network)
  }
  else if (bcoin.script.isPubkeyhashInput(s)) {
    return utils.hash2addr(utils.ripesha(s[1]), config.network)
  }
  else if (bcoin.script.isScripthash(s)) {
    return utils.hash2scriptaddr(utils.ripesha(s[1]), config.network)
  }
}
