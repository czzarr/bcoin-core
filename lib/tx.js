...
var protocol = bcoin.protocol
var utils = bcoin.utils
var assert = utils.assert;

function TX (network, data, block) {
    if (!(this instanceof TX)) return new TX(network, data, block);
    this.type = 'tx';
    ...
    if (data.outputs) {
      for (var i = 0; i < data.outputs.length; i++) {
        this.out(data.outputs[i], null);
      }
      var init = new bn(0)
      this.value = this.outputs.reduce(function (sum, out) {
        return sum.add(out.value)
      }, init)
    }
...
TX.prototype.clone = function clone() {
    return new TX(this.network, this);
};
...
TX.prototype.render = function render() {
    return protocol.framer.tx(this);
};

TX.prototype._input = function _input(i, index) {
    if (i instanceof TX) i = {
        tx: i,
        index: index
    };
    else if (typeof i === 'string' || Array.isArray(i)) i = {
        hash: i,
        index: index
    };

    var hash;
    if (i.tx) hash = i.tx.hash('hex');
    else if (i.out) hash = i.out.hash;
    else hash = i.hash;

    if (typeof hash !== 'string') hash = utils.toHex(hash);
...
TX.prototype._inputIndex = function _inputIndex(hash, index) {
    if (hash instanceof TX) hash = hash.hash('hex');
    for (var i = 0; i < this.inputs.length; i++) {
        var ex = this.inputs[i];
        if (ex.out.hash === hash && ex.out.index === index) return i;
    }
...
TX.prototype.out = function out(output, value) {
    //if (output instanceof bcoin.wallet) output = output.getAddress();
    ...
    var s = output.script ? output.script.slice() : [];
    ...
    if (Array.isArray(output.keys || output.address)) {
        ...
        s = ['dup', 'hash160',
        utils.addr2hash(output.address, this.network), 'eqverify', 'checksig'];
    }
...
TX.prototype.getSubscript = function getSubscript(index) {
    ...
    var s = output.script;
    return bcoin.script.subscript(s);
};
...
TX.prototype.subscriptHash = function subscriptHash(index, s, type) {
    ...
    this.parser.constants.hashType[type], 0, 0, 0);
...
TX.prototype.verify = function verify(index, force) {
    ...
    if (!force && this.ts !== 0) return true;

    return this.inputs.every(function(input, i) {
        if (index !== undefined && index !== i) return true;

        if (!input.out.tx) return false;
...
TX.prototype.inputAddrs = function inputAddrs() {
    return this.inputs.filter(function(input) {
        ...
        return utils.hash2addr(hash, this.network);
    });
};
...
TX.prototype.funds = function funds(side) {
    ...
    // Output
    var acc = new bn(0);
    if (this.outputs.length === 0) return acc;
...
TX.fromJSON = function fromJSON(json) {
    ...
    var tx = new TX(this.network, this.parser.parseTX(raw));
    tx.ts = json.ts;
